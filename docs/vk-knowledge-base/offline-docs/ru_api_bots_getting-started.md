# Документация VK: https://dev.vk.com/ru/api/bots/getting-started
...
> Если ранее вы не работали с API ВКонтакте, перед началом работы мы рекомендуем вам прочитать [это руководство](/ru/api/getting-started).

## [](<#Примеры реализации>)Примеры реализации

### [](#PHP)PHP

[VK PHP Бот](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fbotexample), созданный нашим разработчиком на PHP. Для обработки событий используется Callback API, голосовые сообщения генерируются с помощью API [Яндекс SpeechKit](https://vk.com/away.php?to=https%3A%2F%2Ftech.yandex.ru%2Fspeechkit%2F). [Код проекта на GitHub](https://vk.com/away.php?to=https%3A%2F%2Fgithub.com%2FVKCOM%2Fbot-example-php).

### [](#Java)Java

Бот для работы с Youtrack, созданный нашими разработчиками на Java ([Java SDK](/ru/sdk/java)). Для обработки событий используется Callback API. [Код проекта на GitHub](https://vk.com/away.php?to=https%3A%2F%2Fgithub.com%2FVKCOM%2Fvk-java-sdk%2Fwiki%2FYouTrack-bot).

### [](#JS)JS

Библиотека для работы с ботами на Node JS, автор [Михаил Семин](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fbifot). Используется Long Poll, предусмотрена возможность быстро и удобно добавлять новые команды и реакции. [Код проекта на GitHub](https://vk.com/away.php?to=https%3A%2F%2Fgithub.com%2Fbifot%2Fnode-vk-bot-api).

## [](#Сообщество)Сообщество

Для подключения чат-бота вы можете использовать любое своё сообщество ВКонтакте — группу, встречу или публичную страницу. Ваш бот должен соответствовать правилам, подробнее о них вы можете узнать [здесь](/ru/bots-rules).
...
Также вы можете получить ключ доступа с помощью OAuth. Используйте схему авторизации [Authorization Code Flow](/ru/api/access-token/authcode-flow-user).

### [](<#Добавление ботов в беседы>)Добавление ботов в беседы
...
#### [](<#Только упоминания>)Только упоминания
...
#### [](<#Доступ ко всей переписке>)Доступ ко всей переписке
...
*   •
    ...
    Получать [объект беседы](/ru/reference/objects/conversation)
...
## [](<#Получение событий и обновлений>)Получение событий и обновлений
...
### [](<#Информация о доступных пользователю функциях>)Информация о доступных пользователю функциях
...
Начиная с версии API 5.103 информация о доступных функциях будет приходить вместе с событием [`message_new`](/ru/api/community-events/json-schema) в Callback API и Bots Long Poll API. Она будет находиться в объекте `client_info`:
...
*   •
    ...
    `button_actions` — массив кнопок, которые поддерживает клиент. Возможные значения смотрите в разделе [Клавиатура](/ru/api/bots/development/keyboard#%D0%A1%D1%82%D1%80%D1%83%D0%BA%D1%82%D1%83%D1%80%D0%B0%20%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85);
...
*   •
    ...
    `carousel` (`bool`) — поддерживаются ли [карусели](/ru/api/bots/development/messages) клиентом;
    
*   •
    
    `lang_id` — [ID](/ru/api/api-requests) используемого языка.