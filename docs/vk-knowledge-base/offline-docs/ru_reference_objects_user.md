# Документация VK: https://dev.vk.com/ru/reference/objects/user
...
`boolean` Может ли текущий пользователь видеть профиль при `is_closed` = `1` (например, он есть в друзьях).

## [](<#Опциональные поля A-I>)Опциональные поля A-I

### [](#about)`about`

`string` Содержимое поля «О себе» из профиля.

### [](#activities)`activities`

`string` Содержимое поля «Деятельность» из профиля.

### [](#bdate)`bdate`

`string` Дата рождения. Возвращается в формате D.M.YYYY или D.M (если год рождения скрыт). Если дата рождения скрыта целиком, поле отсутствует в ответе.

### [](#blacklisted)`blacklisted`

`integer` Информация о том, находится ли текущий пользователь в черном списке. Возможные значения:
...
`integer` Информация о том, находится ли пользователь в черном списке у текущего пользователя. Возможные значения:
...
`integer` Информация о том, может ли текущий пользователь оставлять записи на стене. Возможные значения:
...
`integer` Информация о том, может ли текущий пользователь видеть чужие записи на стене. Возможные значения:
...
`integer` Информация о том, может ли текущий пользователь видеть аудиозаписи. Возможные значения:
...
`integer` Информация о том, будет ли отправлено уведомление пользователю о заявке в друзья от текущего пользователя. Возможные значения:
...
`integer` Информация о том, может ли текущий пользователь отправить личное сообщение. Возможные значения:
...
*   •
    ...
    `id` (`integer`) — идентификатор города, который можно использовать для получения его названия с помощью метода [`database.getCitiesById`](/ru/database.getCitiesById);
...
`object` Количество различных объектов у пользователя. Поле возвращается только в методе [`users.get`](/ru/users.get) при запросе информации об одном пользователе, с передачей пользовательского `access_token`. Объект, содержащий следующие поля:
...
`object` Возвращает данные о точках, по которым вырезаны профильная и миниатюрная фотографии пользователя, при наличии.

*   •
    
    `photo` (`object`) — объект [`photo`](/ru/method/photos) фотографии пользователя, из которой вырезается главное фото профиля.
...
### [](#first_name_{case})`first_name_{case}`
...
`string` Содержимое поля «Любимые игры» из профиля.

### [](#has_mobile)`has_mobile`

`integer` Информация о том, известен ли номер мобильного телефона пользователя. Возвращаемые значения: `1` — известен, `0` — не известен.

### [](#has_photo)`has_photo`

`integer` Информация о том, установил ли пользователь фотографию для профиля. Возвращаемые значения: `1` — установил, `0` — не установил.

### [](#home_town)`home_town`

`string` Название родного города.
...
`integer` Информация о том, есть ли пользователь в закладках у текущего пользователя. Возможные значения:
...
`integer` Информация о том, скрыт ли пользователь из ленты новостей текущего пользователя. Возможные значения:
...
`integer` Индексируется ли профиль поисковыми сайтами. Возможные значения:
...
*   •
    ...
    `0` — профиль доступен поисковым сайтам. (В настройках приватности: [https://vk.com/settings?act=privacy](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fsettings%3Fact%3Dprivacy), в пункте «Кому в интернете видна моя страница», выбрано значение «Всем»).
    

### [](#is_verified)`is_verified`

`boolean` Возвращается `true`, если данные пользователя подтверждены, `false` — если нет. Подробнее о подтверждении данных — в [Помощи ВКонтакте](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Ffaq20572).

## [](<#Опциональные поля L-R>)Опциональные поля L-R

### [](#last_name_{case})`last_name_{case}`
...
`string` Разделенные запятой идентификаторы списков друзей, в которых состоит пользователь. Поле доступно только для метода [`friends.get`](/ru/friends.get).
...
`integer` Информация о том, находится ли пользователь сейчас на сайте. Если пользователь использует мобильное приложение либо мобильную версию, возвращается дополнительное поле `online_mobile`, содержащее `1`. При этом, если используется именно приложение, дополнительно возвращается поле `online_app`, содержащее его идентификатор.
...
`string` URL **квадратной** фотографии пользователя, имеющей ширину 50 пикселей. В случае отсутствия у пользователя фотографии возвращается [`https://vk.com/images/camera_50.png`](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fimages%2Fcamera_50.png).

### [](#photo_100)`photo_100`

`string` URL **квадратной** фотографии пользователя, имеющей ширину 100 пикселей. В случае отсутствия у пользователя фотографии возвращается [`https://vk.com/images/camera_100.png`](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fimages%2Fcamera_100.png).

### [](#photo_200_orig)`photo_200_orig`

`string` URL фотографии пользователя, имеющей ширину 200 пикселей. В случае отсутствия у пользователя фотографии возвращается [`https://vk.com/images/camera_200.png`](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fimages%2Fcamera_200.png).

### [](#photo_200)`photo_200`

`string` URL **квадратной** фотографии, имеющей ширину 200 пикселей. Если у пользователя отсутствует фотография таких размеров, в ответе вернется [`https://vk.com/images/camera_200.png`](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fimages%2Fcamera_200.png)

### [](#photo_400_orig)`photo_400_orig`

`string` URL фотографии, имеющей ширину 400 пикселей. Если у пользователя отсутствует фотография такого размера, в ответе вернется [`https://vk.com/images/camera_400.png`](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fimages%2Fcamera_400.png).
...
`string` URL **квадратной** фотографии с максимальной шириной. Может быть возвращена фотография, имеющая ширину как 200, так и 100 пикселей. В случае отсутствия у пользователя фотографии возвращается [`https://vk.com/images/camera_200.png`](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fimages%2Fcamera_200.png).

### [](#photo_max_orig)`photo_max_orig`

`string` URL фотографии максимального размера. Может быть возвращена фотография, имеющая ширину как 400, так и 200 пикселей. В случае отсутствия у пользователя фотографии возвращается [`https://vk.com/images/camera_400.png`](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Fimages%2Fcamera_400.png).
...
`array` Список родственников. Массив объектов, каждый из которых содержит поля:
...
`integer` Семейное положение. Возможные значения:
...
## [](<#Опциональные поля S-W>)Опциональные поля S-W
...
`string` Короткое имя страницы.

### [](#sex)`sex`

`integer` Пол. Возможные значения:
...
`string` Адрес сайта, указанный в профиле.

### [](#status)`status`

`string` Статус пользователя. Возвращается строка, содержащая текст статуса, расположенного в профиле под именем. Если включена опция «Транслировать в статус играющую музыку», возвращается дополнительное поле `status_audio`, содержащее информацию о композиции.

### [](#timezone)`timezone`

`integer` Временная зона. Только при запросе информации о текущем пользователе.

### [](#trending)`trending`

`integer` Информация о том, есть ли на странице пользователя «огонёк».
...
`integer` Возвращается `1`, если страница пользователя верифицирована, `0` — если нет. Подробнее о верификации — в [Помощи ВКонтакте](https://vk.com/away.php?to=https%3A%2F%2Fvk.com%2Ffaq18065).

### [](#wall_default)`wall_default`

`string` Режим стены по умолчанию. Возможные значения: `owner`, `all`.