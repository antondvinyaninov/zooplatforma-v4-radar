...
import "./dist-uu3Q44o-.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
import { t as require_index_umd } from "./@vkontakte_vk-bridge.js";
import { _t as Link } from "./dist-DjyfQdG6.js";
...
function createKey() {
	...
	return Math.random().toString(allNumbersAndLetters).substring(positionAfterZeroAnDot, positionAfterZeroAnDot + 7);
}
...
var BridgeService = class BridgeService {
	...
	static enableNativeSwipeBack() {
		Promise.all([import_index_umd.default.send("VKWebAppSetSwipeSettings", { history: true }), import_index_umd.default.send("VKWebAppEnableSwipeBack")]).catch(BridgeService.handlePlatformError);
	...
	static disableNativeSwipeBack(consumerId) {
		...
		*/
		if (instance.swipeBackConsumers.length === 0) Promise.all([import_index_umd.default.send("VKWebAppSetSwipeSettings", { history: false }), import_index_umd.default.send("VKWebAppDisableSwipeBack")]).catch(BridgeService.handlePlatformError);
	}
	static get instance() {
...
function RouterProvider({ router, children, notFound, hierarchy, notFoundRedirectPath, interval = 400, useBridge = true, throttled = true }) {
	...
	(0, import_react.useEffect)(() => {
		...
		if (useBridge) {
			import_index_umd.default.subscribe(({ detail }) => {
			...
			router.subscribe((state) => {
				...
				import_index_umd.default.send("VKWebAppSetLocation", {
					location,
					replace_state: true
				});
			});
	...
	if (notFoundRedirectPath && (routeNotFound || routeContext.match?.route.path === "*")) if (router.state.location.pathname === notFoundRedirectPath) console.warn("Incorrect notFoundRedirectPath");
